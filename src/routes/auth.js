...
export const registrarUsuario = async (req, res) => {
    ...
    try {
      ...
      if (loginError) {
        ...
        return res.status(500).json({ error: `Error al iniciar sesión: ${loginError.message}` });
      }
  
      console.log("Datos devueltos por signInWithPassword:", loginData);
  
      console.log("Registro exitoso. Enviando respuesta...");
      return res.status(201).json({
        success: `Registro exitoso. Verifica el correo enviado a ${correo}`,
...
export const loginUser = async (req, res) => {
  ...
  try {
    ...
    if (error) {
      const mensajeError = error.message.includes("Invalid login credentials")
